...
class structure(object):
    """structure
    The structure class can hold various substructures.
    Each substructure can be either a layer of N unitCell objects or a structure by itself.
    ...
    def __init__(self, name, **kwargs):
        ...
        numSubSystems = 1;  % INTEGER number of subsystems for heat and phonons
    ...
    def addSubStructure(self, subStructure, N):
        ...
        if not (isinstance(subStructure, unitCell) or isinstance(subStructure, structure)):
            raise ValueError('Class ' + type(subStructure).__name__ +
        ...
        for i in range(len(self.substructures)):
            if isinstance(self.substructures[i][0], unitCell):
                ...
                if not UCIDs:
                    ...
                    # the first unitCell
                    UCIDs = UCIDs + [ID]
                    UCHandles = UCHandles + [self.substructures[i][0]]
                else:
    ...
    def getUnitCellVectors(self, *args):
        ...
        for i in range(len(self.substructures)):
            if isinstance(self.substructures[i][0], unitCell):
                ...
                # the IDs cell array
                temp1 = list(itertools.repeat(self.substructures[i][0].ID,
                                              self.substructures[i][1]))
